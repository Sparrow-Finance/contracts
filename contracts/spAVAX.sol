    ...
    UUPSUpgradeable 
{
    ...
    event UnlockRequested(address indexed user, uint256 spAvaxAmount, uint256 avaxAmount, uint256 unlockTime);
    ...
    event WithdrawalNFTClaimed(address indexed user, uint256 indexed tokenId, uint256 avaxAmount);
    ...
    function totalAssets() public view override returns (uint256) {
        ...
        uint256 committed = accumulatedDaoFees + accumulatedDevFees + totalLockedInUnlocks;
    ...
    function depositAVAX(address receiver) 
        public 
        payable 
        whenNotPaused 
        returns (uint256 shares) 
    ...
    function mintAVAX(uint256 shares, address receiver)
        public
        payable
        whenNotPaused
        returns (uint256 assets)
    ...
    // ============================================
    // NFT WITHDRAWAL CLAIM
    // ============================================
    
    /// @notice Claim AVAX using withdrawal NFT
    function claimWithdrawalNFT(uint256 tokenId) external {
        require(withdrawalQueueNFT.ownerOf(tokenId) == msg.sender, "Not NFT owner");
        
        IWithdrawalQueueNFT.WithdrawalRequest memory request = withdrawalQueueNFT.getRequest(tokenId);
        ...
        require(balance >= request.avaxAmount + fees, "Insufficient liquidity");
        
        // Burn NFT
        withdrawalQueueNFT.burn(tokenId);
        
        // ✅ FIX: Update accounting (MUST match V1 claimUnlock logic)
    ...
    /// @notice Cancel withdrawal NFT and get spAVAX back (before expiry)
    function cancelWithdrawalNFT(uint256 tokenId) external {
        ...
        // ✅ Update accounting (same as V1 cancelUnlock)
        totalLockedInUnlocks -= request.avaxAmount;
        
        // Mint spAVAX back at current rate (user benefits/loses from rate change)
    ...
    /// @notice Claim expired NFT and get spAVAX back at current rate
    function claimExpiredNFT(uint256 tokenId) external {
        ...
        // ✅ Update accounting (same as V1 claimExpired)
        totalLockedInUnlocks -= request.avaxAmount;
        
        // Mint spAVAX back at current rate
        uint256 currentShares = _convertToShares(request.avaxAmount, Math.Rounding.Floor);
        _mint(msg.sender, currentShares);
    ...
    function stake(uint256 minSpAvaxOut) external payable whenNotPaused returns (uint256) {
    ...
    function requestUnlock(uint256 spAvaxAmount, uint256 minAvaxOut) external returns (uint256) {
        require(spAvaxAmount > 0, "Zero amount");
        require(balanceOf(msg.sender) >= spAvaxAmount, "Insufficient balance");
        require(unlockRequests[msg.sender].length < 100, "Too many pending requests");
        ...
        emit UnlockRequested(msg.sender, spAvaxAmount, avaxAmount, block.timestamp + unlockPeriod);
    ...
    function claimUnlock(uint256 index) external {
        ...
        // ✅ FIX: Update accounting (MUST match V1)
        totalPooledAVAX -= request.avaxAmount;
        totalLockedInUnlocks -= request.avaxAmount;
        ...
        ) 
    {
        ...
        return (request.spAvaxAmount, request.avaxAmount, request.unlockTime, request.expiryTime);
    }
    ...
    function withdraw(uint256 amount) external onlyGovernance {
        ...
        require(balance >= committed + amount, "Insufficient liquidity after commitments");
    ...
    function setFeeStructure(uint256 _daoFeeBasisPoints, uint256 _devFeeBasisPoints) 
        external 
        onlyGovernance 
    {
        require(_daoFeeBasisPoints <= 10000, "DAO fee too high");
        require(_devFeeBasisPoints <= 10000, "Dev fee too high");
        require(_daoFeeBasisPoints + _devFeeBasisPoints <= 1000, "Total fees too high (max 10%)");
    ...
    function setMinStakeAmount(uint256 _minStakeAmount) external onlyGovernance {
    ...
    function previewUnlock(uint256 spAvaxAmount) external view returns (uint256) {
