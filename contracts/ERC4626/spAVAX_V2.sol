 ...
 * Exchange Rate Lock: AVAX amount locked at requestUnlock() time
 *
    ...
    UUPSUpgradeable
{
    ...
    function initialize() public initializer {
        ...
        // Note: We don't call __ERC4626_init() because AVAX is native (not ERC20)
    ...
    /**
     * @notice Returns the address of the underlying asset (AVAX)
     * @dev AVAX is native, so we return address(0) as a special case
     */
    function asset() public view virtual override returns (address) {
        return address(0); // AVAX is native, not an ERC20
    }

    /**
     * @notice Returns total assets under management (total AVAX in vault)
     */
    ...
    function decimals() public view virtual override(ERC20Upgradeable, ERC4626Upgradeable) returns (uint8) {
        return 18; // AVAX has 18 decimals
    }
    ...
    /**
     * @notice Stake AVAX and receive spAVAX tokens (original function)
    ...
    function stake(uint256 minSpAvaxOut) external payable nonReentrant whenNotPaused returns (uint256 spAvaxAmount) {
        return _stakeFor(msg.sender, msg.value, minSpAvaxOut);
    }

    /**
     * @notice ERC4626 deposit - NOT SUPPORTED for native AVAX
     * @dev Use stake() instead for native AVAX deposits
    ...
    function deposit(uint256 /* assets */, address /* receiver */) public virtual override nonReentrant whenNotPaused returns (uint256 /* shares */) {
        revert("Use stake() for native AVAX - ERC4626 deposit expects ERC20");
    }

    /**
     * @notice ERC4626 mint - NOT SUPPORTED for native AVAX
     * @dev Use stake() instead for native AVAX deposits
     * @dev ERC4626 standard expects ERC20 tokens, not native assets
     */
    function mint(uint256 /* shares */, address /* receiver */) public virtual override nonReentrant whenNotPaused returns (uint256 /* assets */) {
        revert("Use stake() for native AVAX - ERC4626 mint expects ERC20");
    }

    /**
     * @notice Internal helper - stake AVAX for a specific receiver (DRY principle)
     * @param receiver Address to receive spAVAX tokens
     * @param avaxAmount Amount of AVAX to stake
     * @param minSpAvaxOut Minimum spAVAX to receive (0 = no check)
     * @return spAvaxAmount Amount of spAVAX minted
     */
    function _stakeFor(address receiver, uint256 avaxAmount, uint256 minSpAvaxOut) internal returns (uint256 spAvaxAmount) {
        require(avaxAmount >= minStakeAmount, "Below minimum stake");
        require(receiver != address(0), "Invalid receiver");
        
        if (totalSupply() == 0) {
            spAvaxAmount = avaxAmount;
        } else {
            // Simplified: removed redundant PRECISION multiplication/division
            spAvaxAmount = (avaxAmount * totalSupply()) / totalPooledAVAX;
        }
        ...
        _mint(receiver, spAvaxAmount);

        emit Staked(receiver, avaxAmount, spAvaxAmount);
        return spAvaxAmount;
    }
     ...
     * @return avaxAmount Amount of AVAX to receive (locked rate)
    ...
    function previewStake(
        uint256 avaxAmount
    ) public view returns (uint256 spAvaxAmount) {
        if (totalSupply() == 0 || totalPooledAVAX == 0) {
            return avaxAmount;
        }
    ...
    function previewUnlock(
        uint256 spAvaxAmount
    ) public view returns (uint256 avaxAmount) {
        if (totalSupply() == 0) {
            return 0;
        }
     ...
     * @dev Only governance can send AVAX directly (prevents accidental donations)
     */
